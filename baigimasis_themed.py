...
# -------------------------------------------------------------------------------------------------
# Remeliai
remelis1 = ttk.LabelFrame(root, text="Excel Data")
remelis1.place(height=600, width=700)
remelis2 = ttk.LabelFrame(root, text="Editor")
remelis2.place(height=500, width=750, rely=0, relx=0.48)
file_remelis1 = ttk.LabelFrame(root, text="Plot menu")
file_remelis1.place(height=200, width=750, rely=0.70, relx=0.48)
file_remelis2 = ttk.LabelFrame(root, text="Open File")
file_remelis2.place(height=100, width=700, rely=0.839, relx=0.003)
remelis3 = ttk.LabelFrame(remelis2, text="Excel Data")
remelis3.place(height=300, width=730, rely=0, relx=0.01)
remelis33 = ttk.LabelFrame(remelis2, text="Options")
remelis33.place(height=170, width=730, rely=0.63, relx=0.01)

# Mygtukai
mygtukas1 = ttk.Button(file_remelis2, text="Browse A File", command=lambda: failo_narsykle())
mygtukas1.place(rely=0.65, relx=0.50)
mygtukas2 = ttk.Button(file_remelis2, text="Load File", command=lambda: ikelti_failus(tree1))
mygtukas2.place(rely=0.65, relx=0.30)
editorbutton = ttk.Button(remelis33, text="Open file in editor", command=lambda: [ikelti_failus(tree2), pasirinkt()])
editorbutton.place(rely=0.75, relx=0.45)
mygtukas2 = ttk.Button(file_remelis2, text="Help", command=lambda: help())
...
# Slankikliai
treescrolly = ttk.Scrollbar(remelis1, orient="vertical", command=tree1.yview)
treescrollx = ttk.Scrollbar(remelis1, orient="horizontal", command=tree1.xview)
tree1.configure(xscrollcommand=treescrollx.set, yscrollcommand=treescrolly.set)
treescrollx.pack(side="bottom", fill="x")
treescrolly.pack(side="right", fill="y")

treescrolly2 = ttk.Scrollbar(remelis3, orient="vertical", command=tree2.yview)
treescrollx2 = ttk.Scrollbar(remelis3, orient="horizontal", command=tree2.xview)
...
# -------------------------------------------------------------------------------------------------
def show():
    mygtukas3 = ttk.Button(file_remelis1, text="Show PlotMenu",
                           command=lambda: [diagrama(), mygtukas3.destroy(), label.destroy()])
    mygtukas3.place(rely=0.8, relx=0.01)
    label = ttk.Label(file_remelis1, text="Caution: Big datasets could take a while to create a plot\n"
...
def diagrama():
    try:
        ...
        tree6.append("None")
        xlabel = ttk.Label(file_remelis1, text="Choose X:")
        xlabel.place(rely=0.03, relx=0.01)
        ylabel = ttk.Label(file_remelis1, text="Choose Y:")
        ylabel.place(rely=0.23, relx=0.01)
        huelabel = ttk.Label(file_remelis1, text="Choose Hue:")
        huelabel.place(rely=0.43, relx=0.01)
        plotlabel = ttk.Label(file_remelis1, text="Choose Plot:")
        plotlabel.place(rely=0.63, relx=0.01)
        clickedx = tk.StringVar()
        clickedx.set("None")
        drop1 = ttk.OptionMenu(file_remelis1, clickedx, *tree6)
        drop1.place(rely=0, relx=0.13)
        clickedy = tk.StringVar()
        clickedy.set("None")
        drop2 = ttk.OptionMenu(file_remelis1, clickedy, *tree6)
        drop2.place(rely=0.2, relx=0.13)
        clickedhue = tk.StringVar()
        clickedhue.set("None")
        drop3 = ttk.OptionMenu(file_remelis1, clickedhue, *tree6)
        drop3.place(rely=0.4, relx=0.13)
        ...
        drop4 = ttk.OptionMenu(file_remelis1, clicked1, *pasirinkimai)
        drop4.place(rely=0.6, relx=0.13)

        mygtukas = ttk.Button(file_remelis1, text="Clear OptionMenu", command=lambda: [istrink(), show()])
        ...
        diagramosbutton = ttk.Button(file_remelis1, text="Make a plot", command=lambda: gauti())
...
# -------------------------------------------------------------------------------------------------
def pasirinkt():
    add_button = ttk.Button(remelis33, text="Add Cell", command=lambda: add_cell())
    add_button.place(rely=0.05, relx=0.70)
    labelis = ttk.Label(remelis33, text="Search: ")
    labelis.place(rely=0.065, relx=0.03)
    find = ttk.Button(remelis33, text="Find", command=lambda: find_object())
    find.place(rely=0.05, relx=0.4)
    delete = ttk.Button(remelis33, text="Delete", command=lambda: delete_cell())
    delete.place(rely=0.05, relx=0.55)
    export_button = ttk.Button(remelis33, text="Save to CSV", command=lambda: export_csv())
    export_button.place(rely=0.75, relx=0.65)
    filter_button = ttk.Button(remelis33, text="Show only", command=filter_data)
    filter_button.place(rely=0.05, relx=0.85)
    entry = ttk.Entry(remelis33)
